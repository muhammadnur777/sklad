...
class ZoneMiddleware:
    ...
    def __call__(self, request):
        path = request.path
        skip_paths = ['/login/', '/logout/', '/admin/', '/api/', '/bozor/login/']
        if any(path.startswith(p) for p in skip_paths):
            return self.get_response(request)

        if not request.user.is_authenticated:
            # Если идёт на бозор — перекидываем на бозор логин
            if path.startswith('/bozor/'):
        ...
        is_bozor_page = path.startswith('/bozor/')
        is_sklad_page = not is_bozor_page
